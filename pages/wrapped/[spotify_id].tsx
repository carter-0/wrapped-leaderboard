...
export default function WrappedProfile({ data }: Props) {
    ...
    const { data: soulmateData, isLoading } = useSWR<{ status: string, data: SoulmateData }>(
        `http://localhost:5001/wrapped/${data.user.spotify_id}/soulmate`,
        fetcher
    )
    ...
    const { data: globalLeaderboard } = useSWR<{ status: string, data: LeaderboardEntry[] }>(
        `http://localhost:5001/wrapped/leaderboard?limit=${limit}&offset=${globalOffset}`,
        fetcher
    )

    const { data: genreLeaderboard } = useSWR<{ status: string, data: LeaderboardEntry[] }>(
        `http://localhost:5001/wrapped/leaderboard?genre=${encodeURIComponent(data.top_genre)}&limit=${limit}&offset=${genreOffset}`,
        fetcher
    )

    const { data: artistLeaderboard } = useSWR<{ status: string, data: LeaderboardEntry[] }>(
        `http://localhost:5001/wrapped/leaderboard?artist=${encodeURIComponent(data.top_artists[0])}&limit=${limit}&offset=${artistOffset}`,
        fetcher
    )
...
export const getServerSideProps: GetServerSideProps = async ({ params }) => {
    ...
    try {
        const response = await fetch(`http://localhost:5001/wrapped/${spotify_id}`)
